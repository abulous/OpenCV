...
video2 = cv2.VideoCapture("roadO.mov")
...
while(True):
    ...
    (ret2, play2) = video2.read()

 
    play = cv2.cvtColor(play, cv2.COLOR_BGR2GRAY)
    play1 = cv2.cvtColor(play1, cv2.COLOR_BGR2GRAY)
    play2 = cv2.cvtColor(play2, cv2.COLOR_BGR2GRAY)
...
# Video Loop
    ...
    loop1 +=1
    if loop1 == video1.get(7):
        video1.set(1, 0)
        loop1 = 0

    loop2 +=1
    if loop2 == video2.get(7):
        video2.set(1, 0)
        loop2 = 0
    # Convert BGR to HSV
    ...
    vidBlend = cv2.addWeighted(play, 0.5, play1, 0.5, 0)
##    capBlend = cv2.addWeighted(frame, 0.5, dframe, 0.5, 0)
    capBlend = cv2.addWeighted(frame, 0.5, play2, 0.5, 0)
    blendAll = cv2.addWeighted(capBlend, weight1, vidBlend, weight2, 0)
    # show the frame to our screen
    cv2.imshow("Frame", blendAll)
